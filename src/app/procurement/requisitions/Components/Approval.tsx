...
function ApprovalForm({ user, data, officer, isOpen, ...props }: Props3) {
	...
	return (
		<>
			...
			<form action={submitForm} className="block w-full">
				<input hidden name="approval_id" defaultValue={data.id} />
				<div className="grid grid-cols-4 w-full gap-4 pt-4">
					<div className="grid gap-2">
						<Label htmlFor="approve">Do You Approve</Label>
						<Switch id="approve" name="approve" />
					</div>
				...
				<div className="pt-8 p-6 max-w-[400px] w-full mx-auto">
					<SubmitButton className="w-full" text="Submit Approval" />
				</div>
			</form>
		</>
	);
				...
				>
					<SelectTrigger
						title={
							partOfAnnualPlan
								? ""
								: "It must be part of a annual plan before selecting a plan"
						}
