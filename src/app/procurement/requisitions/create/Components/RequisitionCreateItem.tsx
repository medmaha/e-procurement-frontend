...
type Props = {
  ...
  updateTotalCost: (total: number) => void;
};

export default function RequisitionCreateItem({
  idx,
  data,
  remove,
  ...props
  ...
  function updateTotalCost() {
    ...
    if (!isNaN(Number(totalPriceHidden.current?.value))) {
      props.updateTotalCost(Number(totalPriceHidden.current?.value));
    } else {
      props.updateTotalCost(0);
    }
  }
