	...
	return (
		<>
			<header className="h-[71px] min-w-full lg:w-[280px]">
				<div className="flex items-center gap-4 justify-between sm:justify-center h-full border-b border-r px-4">
					<h2 className="text-xl md:text-2xl lg:text-3xl font-bold pb-[2px]">
						<a
...
export function Accordions({ user, closeNav }: any) {
	...
	function getLinkColor(href: any) {
		if (href === state.link.href) {
			return "link underline underline-offset-[5px] underline-secondary";
		}
		return "";
	}
					...
					>
						<AccordionTrigger
							className={`${getTabColor(
								accordion
							)} font-semibold text-base sm:text-xl py-1.5 my-2 px-2 rounded-md`}
						>
							{accordion.label}
						</AccordionTrigger>
						<AccordionContent className="pt-0 mt-0 ml-6">
											...
											<Link
												...
												className={`${getLinkColor(item.href)} ${
													item.name === "procurement_thresholds"
														? "text-sm"
														: ""
												} hover:text-primary text-sm sm:text-base text w-full`}
											>
												{item.label}
											</Link>
										</li>
